...
static struct gpsfile_priv *
gpsfile_create(char *name, int id, char *source)
{
	struct gpsfile_priv *ret;
...
static int load_config(int reload)
{
	...
	while ((cat = navit_cfg_cats_walk(cfg, cat))) {
		...
		if (var && (!strcmp(cfg_var_value(var), "file")||!strcmp(cfg_var_value(var), "pipe"))) {
...
static int filegps_reconfigure(void)
{
	ENTER();
	load_config(1);
	return M_OK;
...
static int filegps_unload(void)
{
	ENTER();
	return M_OK;
}

NAVIT_MODULE(MODNAME, filegps_load,filegps_reconfigure,filegps_unload);
