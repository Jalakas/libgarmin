...
int gar_load_rgnpoints(struct gar_subfile *sub)
{
	...
	int idx = 1;
	struct gimg *g = sub->gimg;
	int off = sub->rgnbase + sub->rgnoffset4;
	struct gpoint *gp;
	...
	while (rc < sub->rgnlen4) {
		rc += gread(g, buf, sizeof(buf));
		sta = gar_parse_point(buf, &gp);
		if (sta > 0) {
			gp->n = idx++;
			ga_append(&sub->rgn.points, gp);
			log(11, "Point: type: %x %x x=%d y=%d poi=%d st=%d lbl=%08x\n",
					gp->type, gp->subtype,
					gp->c.x, gp->c.y, 
					gp->is_poi, gp->has_subtype,
					gp->lbloffset);
		}
	}
	return idx - 1;
...
int gar_load_rgnpolys(struct gar_subfile *sub)
{
	...
	struct gpoly *poly;
	struct gimg *g = sub->gimg;
	int off = sub->rgnbase + sub->rgnoffset2;
	...
	rc = gread(g, buf, rc);
	cp = buf;
	ep = buf + rc;//sizeof(buf);

	while (cp < ep) {
		if (*cp == 0)
			cp++;
		cp += gar_parse_poly(cp, ep, &poly, 0, &ok, 0);
		if (ok) {
			poly->n = idx++;
			ga_append(&sub->rgn.polygons, poly);
			log(11, "Poly: type: %x x=%d y=%d deltas=%d lbl=%08x\n",
					poly->type, 
					poly->c.x, poly->c.y, 
					poly->npoints,
					poly->lbloffset);
		} else {
			log(11, "Error after: %d\n", cp - buf);
		}
	}
	return idx - 1;
}
...
int gar_load_rgnlines(struct gar_subfile *sub)
{
	...
	struct gpoly *poly;
	struct gimg *g = sub->gimg;
	int off = sub->rgnbase + sub->rgnoffset3;
	...
	while (cp < ep) {
		if (*cp == 0)
			cp++;
		sta = gar_parse_poly(cp, ep, &poly, 1, &ok, 0);
		if (sta > 0 && ok) {
			if (ok) {
				cp+=sta;
				poly->n = idx++;
				ga_append(&sub->rgn.polylines, poly);
				log(11, "line: type: %x x=%d y=%d deltas=%d lbl=%08x\n",
						poly->type, 
						poly->c.x, poly->c.y, 
						poly->npoints,
						poly->lbloffset);
